...
class BannerAndExplorerViewModel(application: Application, handle: SavedStateHandle) : BaseViewModel(
    application,
    handle
) {
    ...
    fun getListBanner() = liveData(Dispatchers.IO) {
        emit(Resource.loading(null))
        try {
            emit(Resource.success(explorerRepositoryAPI.getListBanner()))
        } catch (e: Exception) {
            emit(Resource.error(null, e.message ?: "null"))
        }
    }
